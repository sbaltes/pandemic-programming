...
Sys.setlocale(category = "LC_ALL", "ru_RU")
...
# function definitions (no need to change IF IT WORKS)
match_str <- function(search_str) {
  as.logical(unlist(sapply(filtered[,1], function(input) {
    # print(input)
    # print(search_str)
    grepl(search_str, input, ignore.case = TRUE)
    # print(result)
    # result
  })))
}
...
is_developer <- function() {
  developer_ru <- match_str("разработчик") # you can put the translated version here
  programmer_ru <- match_str("программист") # you can put the translated version here
  sre <- match_str("SRE") # you can put the translated version here
  devops <- match_str("devops") # you can put the translated version here
  devops_ru <- match_str("девопс") # you can put the translated version here
  
  # engineer_other <- FALSE # <- match_str("") # you can put the translated version here
  # result <- developer_en | engineer_en | developer_other | engineer_other
  result <- developer_ru | programmer_ru | sre | devops | devops_ru
  result
}
...
covid_response_mapping <- "'Я совсем не выхожу из дома, ни по какой причине.'=3; 'Я выхожу из дома только по неотложным делам (за продуктами, к врачу, заняться спортом)'=2; 'Я регулярно выхожу из дома по работе или для удовольствия. Я соблюдаю дистанцию от других людей.'=1; 'Я регулярно выхожу из дома. Я НЕ соблюдаю дистанцию от других людей.'=0; else=NA"
yes_no_maybe_mapping <- "'Да'=1; 'Возможно / Не знаю'=0; 'Нет'=-1; else=NA"
likert_scale_mapping_1 <- "'Полностью соглашусь'=4; 'В общем соглашусь'=3; 'Скорее соглашусь'=2; 'Отчасти соглашусь'=1; 'Совсем не соглашусь'=0; 'Не знаю / Не имею мнения на этот счёт'=-99; else=NA"
likert_scale_mapping_2 <- "'Полностью соглашусь'=3; 'Соглашусь'=2; 'Скорее соглашусь'=1; 'Скорее не соглашусь'=-1; 'Не соглашусь'=-2; 'Полностью не соглашусь'=-3; else=NA"
frequency_mapping_1 <- "'Всё время'=5; 'Большую часть времени'=4; 'Чаще да, чем нет'=3; 'Чаще нет, чем да'=2; 'Иногда'=1; 'Никогда'=0; else=NA"
frequency_mapping_2 <- "'Всё время'=4; 'Большую часть времени'=3; 'Часть времени'=2; 'Небольшую часть времени'=1; 'Никогда'=0; else=NA"
performance_mapping <- "'Гораздо лучше, чем у других работников'=3; 'Лучше, чем у других'=2; 'Немного лучше, чем у других'=1; 'Около среднего'=0; 'Немного хуже, чем у других работников'=-1; 'Хуже, чем у других'=-2; 'Гораздо хуже, чем у других'=-3; else=NA"
age_mapping <- "'Меньше 20'=1; '20-24'=2; '25-29'=3; '30-34'=4; '35-39'=5; '40-44'=6; '45-49'=7; '50-54'=8; '55-59'=9; '60-64'=10; '65+'=11; else=NA"
gender_mapping <- "'Женский'='Female'; 'Мужской'='Male'; 'Небинарный / другой'='NB'; 'Другой'='Other'; else=NA"
education_mapping <- "'Кандидат наук, подобное, или выше'=4; 'Степень магистра или эквивалент'=3; 'Степень бакалавра или эквивалент'=2; 'Неоконченное высшее'=1; 'Без высшего образования'=0; else=NA"
organization_size_mapping <- "'0 — 9'=1; '10 — 99'=2; '100 — 999'=3; '1000 — 9999'=4; '10 000 — 99 999'=5; '100 000 или больше'=6; else=NA"
disability_mapping <- "'Да'=2; 'Возможно'=1; 'Нет'=0; else=NA"
# use the following mapping in case you need to translate country names (examples in Turkish)
country_mapping <- "'Россия'='Russia'; 'Украина'='Ukraine'; 'Беларусь'='Belarus'; 'Нидерланды'='Netherlands'; 'Нидерланды '='Netherlands'; 'UK'='United Kingdom'; 'Великобритания'='United Kingdom'; 'Великобритания '='United Kingdom'; 'Бельгия'='Belgium'; 'Australia'='Australia'; 'Германия'='Germany'; 'Германия '='Germany'; 'США'='United States'; 'Сша'='United States'; 'USA'='United States'; 'Ирландия'='Ireland'; 'израиль'='Israel'; ' Израиль'='Israel'; 'Израиль'='Israel'; 'Israel'='Israel'; 'israel'='Israel'; 'Чехия'='Czech Republic'; 'Армения'='Armenia'; 'Netherlands'='Netherlands'; 'Польша'='Poland'; 'Швеция'='Sweden'; 'Россия (Санкт-Петербург) и Германия (Мюнхен)'='Russia and Germany'; 'Latvija'='Latvia'; 'United States'='United States'; 'Кипр'='Cyprus'; 'Тайланд '='Thailand'; else=NA"
...
filtered$COVIDStatus5 <- as.integer(match_str("У меня положительный результат теста на COVID-19"))
filtered$COVIDStatus4 <- as.integer(match_str("У кого-то из тех, с кем я живу, положительный тест на COVID-19"))
filtered$COVIDStatus3 <- as.integer(match_str("У члена моей семьи положительный тест на COVID-19"))
filtered$COVIDStatus2 <- as.integer(match_str("У моего близкого друга положительный тест на COVID-19"))
filtered$COVIDStatus1 <- as.integer(match_str("Я сейчас, или недавно был\\(а\\), в карантине \\(получил\\(а\\) указание ни по какой причине не выходить из дома\\)"))
filtered$COVIDStatus0 <- as.integer(match_str("Ничего из вышеперечисленного"))
...
print_remaining_responses(is_developer())
filtered <- remove_column()
...
# filtered$Country <- paste0(filtered$Country, " >>> ", recode_column(country_mapping, asCharacter=TRUE))
