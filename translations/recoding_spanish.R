...
map_agreement_helpful <- function() {
  ...
  true <- match_str('verdader')
  helpful <- match_str('ayuda')
...
is_developer <- function() {
  developer_en <- match_str(".*[Dd]esarrollador.*")
  engineer_en <- match_str(".*[Ii]ngenier.*")
  developer_other <- FALSE # <- match_str("") # you can put the translated version here
...
is_fulltime <- function() {
  result <- match_str(".*[Tt]iempo completo.*")
  result
}
...
covid_response_mapping <- "'No salgo de casa, por ninguna razón.'=3; 'Salgo de casa solo para tareas esenciales (por ejemplo, hacer la compra, atención médica, ejercicio)'=2; 'Salgo de casa regularmente por trabajo o por placer. Mantengo distancia con los demás.'=1; 'Salgo de casa regularmente. NO estoy manteniendo distancia con los demás.'=0; else=NA"
yes_no_maybe_mapping <- "'Sí'=1; 'Quizás/No estoy seguro'=0; 'No'=-1; else=NA"
likert_scale_mapping_1 <- "'Completamente de acuerdo'=4; 'Muy de acuerdo'=3; 'Algo de acuerdo'=2; 'Un poco de acuerdo'=1; 'Nada de acuerdo'=0; 'No lo sé / No opino'=-99; else=NA"
likert_scale_mapping_2 <- "'Muy de acuerdo'=3; 'De acuerdo'=2; 'Algo de acuerdo'=1; 'Algo de desacuerdo'=-1; 'En desacuerdo'=-2; 'Totalmente en desacuerdo'=-3; else=NA"
frequency_mapping_1 <- "'Todo el tiempo'=5; 'Casi todo el tiempo'=4; 'Más de la mitad del tiempo'=3; 'Menos de la mitad del tiempo'=2; 'Alguna vez'=1; 'En ningún momento'=0; else=NA"
frequency_mapping_2 <- "'Todo el tiempo'=4; 'Casi todo el tiempo'=3; 'Algo de tiempo'=2; 'En algún momento'=1; 'En ningún momento'=0; else=NA"
performance_mapping_1 <- "'Eras mucho mejor que otros trabajadores'=3; 'Eras algo mejor que otros trabajadores'=2; 'Eras un poco mejor que otros trabajadores'=1; 'Estabas en el promedio'=0; 'Eras un poco peor que otros trabajadores'=-1; 'Eras peor que otros trabajadores'=-2; 'Eras mucho peor que otros trabajadores'=-3; else=NA"
performance_mapping_2 <- "'Eres mucho mejor que otros trabajadores'=3; 'Eres algo mejor que otros trabajadores'=2; 'Eres un poco mejor que otros trabajadores'=1; 'Estás en el promedio'=0; 'Eres un poco peor que otros trabajadores'=-1; 'Eres peor que otros trabajadores'=-2; 'Eres mucho peor que otros trabajadores'=-3; else=NA"
age_mapping <- "'Menos de 20'=1; '20-24'=2; '25-29'=3; '30-34'=4; '35-39'=5; '40-44'=6; '45-49'=7; '50-54'=8; '55-59'=9; '60-64'=10; '65+'=11; else=NA"
gender_mapping <- "'Mujer'='Female'; 'Hombre'='Male'; 'No binario / tercer género'='NB'; 'Other'='Other'; else=NA"
education_mapping <- "'Doctorado o equivalente'=4; 'Máster (maestría), licenciatura, ingeniería o equivalente'=3; 'Grado, diplomado, ingeniería técnica o equivalente'=2; 'Alguna educación postsecundaria'=1; 'Ninguna educación postsecundaria'=0; else=NA"
organization_size_mapping <- "'0 a 9'=1; '10 a 99'=2; '100 a 999'=3; '1000 a 9999'=4; '10,000 a 99,999'=5; '100,000 o más'=6; else=NA"
disability_mapping <- "'Sí'=2; 'Quizás'=1; 'No'=0; else=NA"
...
filtered$COVIDStatus5 <- as.integer(match_str("He dado positivo por COVID-19"))
filtered$COVIDStatus4 <- as.integer(match_str("Alguien diferente en mi hogar ha dado positivo por COVID-19"))
filtered$COVIDStatus3 <- as.integer(match_str("Un miembro de mi familia ha dado positivo por COVID-19"))
filtered$COVIDStatus2 <- as.integer(match_str("Un amigo cercano ha dado positivo por COVID-19"))
filtered$COVIDStatus1 <- as.integer(match_str("Actualmente estoy o recientemente ha estado en cuarentena \\(se me ordenó no salir de casa por algún motivo\\)"))
filtered$COVIDStatus0 <- as.integer(match_str("Ninguna de las anteriores"))
